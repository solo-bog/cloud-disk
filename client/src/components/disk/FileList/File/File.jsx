...
const File = ({file, viewType}) => {
  ...
  const setCurrentDirHandler = (newDir, prevDir, newDirName) => {
    dispatch(setCurrentDir(newDir));
    dispatch(dirPush(prevDir, newDirName));
  };
  return (
    <div className='file'>
      <img src={file.type === 'dir' ? dirLogo : fileLogo} alt="" className="file__img"/>
      <div className="file__name" title={file.name} onClick={file.type === 'dir'? () => setCurrentDirHandler(file._id, file.parent, file.name):null}>{file.name}</div>
      {viewType == 'list' &&
        <React.Fragment>
